...
func RunOrchestrator(ctx context.Context, req OrchestratorRequest) OrchestratorResponse {
	...
	if req.ResumeFrom != nil && len(req.ResumeFrom.Messages) > 0 {
		...
		messages = []chatMessage{
			{Role: "system", Content: orchestratorInstructions(orchestrator, req.Task, req.Mode)},
		}
	...
	if req.AgentID != "" {
		if a := getAgent(req.AgentID); a != nil {
			start := time.Now()
			out, err := runSubagent(ctx, a, req.Task, model, req.Mode, req.ConvID, req.TenantID, req.History)
	...
	for step := startStep; step <= maxSteps; step++ {
		...
		for _, tc := range respMsg.ToolCalls {
			...
			start := time.Now()
			out, err := runSubagent(ctx, target, subTask, usedModel, req.Mode, req.ConvID, req.TenantID, req.History)
			...
			case "build":
				...
				return OrchestratorResponse{
					Reply: desc + "\n\nSim ou Nao?", Steps: step, ModelUsed: usedModel,
...
func runSubagent(ctx context.Context, a *HOKAgent, task string, model string, mode string, convID string, tenantID string, history []Turn) (string, error) {
	if !a.Active {
	...
	messages := []chatMessage{
		{Role: "system", Content: subagentSystemPrompt(a, task, mode)},
	}
	// Histórico da conversa (mantém contexto entre turnos)
	...
	for step := 1; step <= 5; step++ {
		...
		for _, tc := range respMsg.ToolCalls {
			if tc.Function.Name == "run_engine" {
				switch mode {
				case "plan":
					result := "Modo PLANO: nao posso executar acoes. So posso analisar e planejar. Para executar, o usuario deve mudar para modo Construir ou Autonomo Total."
					messages = append(messages, chatMessage{Role: "tool", ToolCallID: tc.ID, Name: "run_engine", Content: result})
					continue
			case "build":
				if isThinkingMode(convID) {
					break
				}
				desc := describeRunEngineAction(tc.Function.Arguments)
...
func orchestratorInstructions(a *HOKAgent, task string, mode string) string {
	inst := a.Instructions
	if inst == "" {
	...
	planNotice := ""
	if mode == "plan" {
		planNotice = "\n\nMODALIDADE PLANO (PLANO DE TRABALHO APENAS): Voce SO pode ANALISAR e PLANEJAR. NAO pode executar NENHUMA acao via run_engine ou outras ferramentas de execucao. Se solicitado a executar, recuse dizendo: Nao posso executar em modo plano. Mude para modo Construir ou Autonomo Total para executar acoes."
	}
...
- Nunca pare o plano silenciosamente apos uma aprovacao — se ha mais mutacoes pendentes na sequencia original, crie a proxima pending_action imediatamente.%s`, task, inst, planNotice, planNotice)
}

func subagentSystemPrompt(a *HOKAgent, task string, mode string) string {
	inst := a.Instructions
	if inst == "" {
...
func runEngineToolExec(ctx context.Context, argsJSON string) string {
	...
	switch args.Engine {
	case "claude":
		log.Printf("[runEngine] engine claude remapeado para opencode (claude instavel neste ambiente)")
