...
func isThinkingMode(convID string) bool {
	thinkingMu.Lock()
	defer thinkingMu.Unlock()
	return thinkingMap[convID]
}

// setThinkingMode ativa o modo pensamento para uma conversa.
func setThinkingMode(convID string) {
	thinkingMu.Lock()
	defer thinkingMu.Unlock()
	thinkingMap[convID] = true
	log.Printf("[thinking] modo pensamento ATIVADO conv=%s", convID)
}

// clearThinkingMode desativa o modo pensamento.
func clearThinkingMode(convID string) {
	thinkingMu.Lock()
	defer thinkingMu.Unlock()
	delete(thinkingMap, convID)
	log.Printf("[thinking] modo pensamento DESATIVADO conv=%s", convID)
}
...
func isRejectionText(msg string) bool {
	l := strings.ToLower(strings.TrimSpace(msg))
	for _, w := range []string{"nao", "não", "cancela", "cancelar", "para", "rejeita", "stop", "pause"} {
...
func resolveRunEnginePendingAction(ctx context.Context, pa *PendingAction, convID, tenantID string) string {
	...
	// Ativar modo pensamento para execucao contínua sem aprovacoes adicionais
	setThinkingMode(convID)
	...
	if paExistente == nil {
		...
		clearThinkingMode(convID)
		log.Printf("[thinking] modo pensamento ENCERRADO — pendente criada conv=%s", convID)
	}
